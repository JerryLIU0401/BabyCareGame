...
- 擴充既有 `ManualPanelControllerTests`，驗證正在倒數時開啟會暫停、關閉會恢復。
- 驗證尚未開始的計時器經過開啟與關閉後仍未開始。
- 執行 Unity EditMode 測試並確認 C# 編譯無錯誤。
- 在 Unity Editor 手動確認倒數期間開啟說明書，等待數秒後數值不變，返回後從原數值繼續。
- 確認 `GameSetting.prefab` 的說明書開啟、翻頁、返回功能未受影響。
